...
import type { Card as CardType, Comment as CommentType } from "@/shared/types";
...
export const CardModal = ({ card, isOpen, onClose }: CardModalProps) => {
  ...
  const { updateCardTitle, deleteCard } = useCardActions();
  const { addComment } = useCommentActions();

  const previousCardIdRef = useRef<string | null>(null);
  const cardId = card?.id ?? null;

  // Use state for editing, derive from card when not editing
  ...
  // Derive title from card when not editing, use editingTitle when editing
  const title = isEditingTitle ? editingTitle : currentCard?.title || "";

  // Reset state when card changes using useLayoutEffect (runs synchronously after render)
  useLayoutEffect(() => {
    if (cardId && cardId !== previousCardIdRef.current && card) {
